    ...
    if (canvas) {
        ...
        async function loadChartData() {
            ...
            try {
                ...
                const res = await fetch(`https://api.coingecko.com/api/v3/coins/${primaryCoin}/market_chart?vs_currency=usd&days=7`, { headers });
                ...
                let hash = 0;
                for (let i = 0; i < primaryCoin.length; i++) hash = primaryCoin.charCodeAt(i) + ((hash << 5) - hash);
                const basePrice = Math.abs(hash) % 10000 + 100;
                const prices = labels.map((_, i) => basePrice + (Math.sin(i) * basePrice * 0.1));
                
                portfolioChart.data.labels = labels;
                portfolioChart.data.datasets[0].data = prices;
                const capitalize = s => s.charAt(0).toUpperCase() + s.slice(1);
                portfolioChart.data.datasets[0].label = `${capitalize(primaryCoin)} Price ($) (Mocked)`;
                portfolioChart.update();

                const chartTitle = document.getElementById('chart-title');
                if(chartTitle) {
                    chartTitle.innerText = `${capitalize(primaryCoin)} - 7 Day Trend (Mocked)`;
                }
            }
        }
